...
@Component({
  selector: 'app-mediano',
  templateUrl: './mediano.html',
  styleUrls: ['./mediano.css'],
  standalone: false
})
export class MedianoComponent implements OnInit {
  ...
  public construirRutaImagen(item: any, nombreArchivo: string | undefined): string {
    ...
    const tam = item.tamano ? `${item.tamano.toLowerCase()}/` : 'mediano/'; 
  ...
  try {
    const { data, error } = await this.supabaseService.client
      ...
      .eq('tamano', 'MEDIANO') 
      .gte('variantes_stock.talla', 34)
      .lte('variantes_stock.talla', 38)
...
get productosPaginados() {
  const filtrados = this.productos.filter(item => {
    ...
    const tieneTallaMediano = item.tallas_disponibles.some((t: any) => {
      const n = Number(t);
      return n >= 34 && n <= 38;
    });
    return esZapatilla && tieneTallaMediano;
  });
...
get totalPaginas() {
  const filtrados = this.productos.filter(item => {
    const esZapatilla = item.tamano !== 'ACCESORIO';
    const tieneTallaMediano = item.tallas_disponibles.some((t: any) => Number(t) >= 34 && Number(t) <= 38);
    return esZapatilla && tieneTallaMediano;
  });
