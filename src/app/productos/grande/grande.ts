...
export class GrandeComponent implements OnInit {
  ...
  public construirRutaImagen(item: any, nombreArchivo: string | undefined): string {
    ...
    const tam = item.tamano ? `${item.tamano.toLowerCase()}/` : 'grande/'; 
  ...
  try {
    const { data, error } = await this.supabaseService.client
      ...
      .eq('tamano', 'GRANDE') 
      .gte('variantes_stock.talla', 39)
      .lte('variantes_stock.talla', 43)
...
get productosPaginados() {
  const filtrados = this.productos.filter(item => {
    ...
    const tieneTallaGrande = item.tallas_disponibles.some((t: any) => {
      const n = Number(t);
      return n >= 39 && n <= 43;
    });
    return esZapatilla && tieneTallaGrande;
  });
...
get totalPaginas() {
  const filtrados = this.productos.filter(item => {
    const esZapatilla = item.tamano !== 'ACCESORIO';
    const tieneTallaGrande = item.tallas_disponibles.some((t: any) => Number(t) >= 39 && Number(t) <= 43);
    return esZapatilla && tieneTallaGrande;
  });
