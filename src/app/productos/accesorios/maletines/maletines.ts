...
@Component({
  selector: 'app-maletines',
  templateUrl: './maletines.html',
  styleUrls: ['./maletines.css'],
  standalone: false
})
export class MaletinesComponent implements OnInit {
  ...
  public construirRutaImagen(item: any, nombreArchivo: string | undefined): string {
    ...
    const carpetaBase = 'ACCESORIOS/MALETINES/'; //
  ...
  async ngOnInit() {
    try {
      ...
      console.error("Error Ivano Maletines:", err);
    } finally {
      this.loading = false;
