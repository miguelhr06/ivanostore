...
@Component({
  selector: 'app-tarjeteros',
  templateUrl: './tarjeteros.html',
  styleUrls: ['./tarjeteros.css'],
  standalone: false
})
export class TarjeterosComponent implements OnInit {
  ...
  public construirRutaImagen(item: any, nombreArchivo: string | undefined): string {
    ...
    const carpetaBase = 'ACCESORIOS/TARJETEROS/'; //
  ...
  async ngOnInit() {
    try {
      ...
      console.error("Error Ivano Tarjeteros:", err);
    } finally {
      this.loading = false;
