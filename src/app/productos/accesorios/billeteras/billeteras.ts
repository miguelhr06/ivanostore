...
@Component({
  ...
  templateUrl: './billeteras.html',
  styleUrls: ['./billeteras.css'],
  standalone: false
})
export class BilleterasComponent implements OnInit {
  ...
  public construirRutaImagen(item: any, nombreArchivo: string | undefined): string {
    ...
    const carpetaBase = 'ACCESORIOS/BILLETERAS/'; //
  ...
  async ngOnInit() {
    try {
      ...
      console.error("Error Ivano Billeteras:", err);
    } finally {
      this.loading = false;
