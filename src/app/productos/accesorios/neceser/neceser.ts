...
@Component({
  selector: 'app-neceser',
  templateUrl: './neceser.html',
  styleUrls: ['./neceser.css'],
  standalone: false
})
export class NecesersComponent implements OnInit {
  ...
  public construirRutaImagen(item: any, nombreArchivo: string | undefined): string {
    ...
    const carpetaBase = 'ACCESORIOS/NECESERS/'; //
  ...
  async ngOnInit() {
    try {
      ...
      console.error("Error Ivano Necesers:", err);
    } finally {
      this.loading = false;
