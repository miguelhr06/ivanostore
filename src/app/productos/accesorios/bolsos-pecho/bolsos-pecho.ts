...
@Component({
  selector: 'app-bolsos-pecho',
  templateUrl: './bolsos-pecho.html',
  styleUrls: ['./bolsos-pecho.css'],
  standalone: false
})
export class BolsosPechoComponent implements OnInit {
  ...
  public construirRutaImagen(item: any, nombreArchivo: string | undefined): string {
    ...
    const carpetaBase = 'ACCESORIOS/BOLSOS_PECHO/'; //
  ...
  async ngOnInit() {
    try {
      ...
      console.error("Error Ivano Bolsos de Pecho:", err);
    } finally {
      this.loading = false;
