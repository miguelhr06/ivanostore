...
@Component({
  ...
  styleUrls: ['./mochilas.css'],
  standalone: false
})
export class MochilasComponent implements OnInit {
  ...
  async ngOnInit() {
    try {
      const { data, error } = await this.supabaseService.client
        ...
        .or(`categoria.eq.MOCHILAS, sub_categoria.cs.{MOCHILAS}`);
      ...
      console.error("Error Ivano Mochilas:", err);
    } finally {
      this.loading = false;
