...
@Component({
  selector: 'app-correas',
  templateUrl: './correas.html',
  styleUrls: ['./correas.css'],
  standalone: false
})
export class CorreasComponent implements OnInit {
  ...
  public construirRutaImagen(item: any, nombreArchivo: string | undefined): string {
    ...
    const carpetaBase = 'ACCESORIOS/CORREAS/'; //
  ...
  async ngOnInit() {
    try {
      ...
      console.error("Error Ivano Correas:", err);
    } finally {
      this.loading = false;
