...
@Component({
  selector: 'app-canguros',
  templateUrl: './canguros.html',
  styleUrls: ['./canguros.css'],
  standalone: false
})
export class CangurosComponent implements OnInit {
  ...
  public construirRutaImagen(item: any, nombreArchivo: string | undefined): string {
    ...
    const carpetaBase = 'ACCESORIOS/CANGUROS/'; //
  ...
  async ngOnInit() {
    try {
      ...
      console.error("Error Ivano Canguros:", err);
    } finally {
      this.loading = false;
